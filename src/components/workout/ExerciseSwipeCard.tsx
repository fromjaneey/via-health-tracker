...
import { Check, Info, AlertTriangle, Play, ChevronDown, ChevronUp, Repeat } from "lucide-react";
...
const ExerciseSwipeCard = ({
  ...
  const swapLabelOpacity = useTransform(x, [-200, -60, 0, 60, 200], [1, 1, 0, 1, 1]);

  const alternatives = getAlternatives(exercise, userEquipment);
  const allDone = sets.every((s) => s.completed);

  const handleDragEnd = (_: any, info: PanInfo) => {
    if (Math.abs(info.offset.x) > 100 && alternatives.length > 0) {
  ...
  return (
    <div className="relative">
      {/* Swap hint background */}
      {alternatives.length > 0 && (
        <motion.div
          style={{ opacity: swapLabelOpacity }}
      ...
      <motion.div
        ...
        drag={alternatives.length > 0 ? "x" : false}
        dragConstraints={{ left: 0, right: 0 }}
        dragElastic={0.7}
        onDragEnd={handleDragEnd}
        className={`bg-card rounded-2xl border transition-colors cursor-grab active:cursor-grabbing ${
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-2 shrink-0">
            {alternatives.length > 0 && (
              <span className="text-[10px] text-muted-foreground bg-muted px-2 py-0.5 rounded-full">swipe to swap</span>
            )}
